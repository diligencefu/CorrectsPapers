...
class PersonalModel: NSObject {
    ...
    ///有没有填写过身份信息
    var haveIdentity : String!
    ///有没有填写过教师资格证信息
    var haveCeltyl : String!
    ///批改科目
    var user_correct_subject : String!
    ///工作时间
    var user_job_time : String!
    ...
    class func setValueForPersonalModel(json: JSON) -> PersonalModel {
        ...
        model.haveIdentity = json["haveIdentity"].stringValue
        model.haveCeltyl = json["haveCeltyl"].stringValue
        model.user_correct_subject = json["user_correct_subject"].stringValue
        model.user_job_time = json["user_job_time"].stringValue
        return model
    }
}
